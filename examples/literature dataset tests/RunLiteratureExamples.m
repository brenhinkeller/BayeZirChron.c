...
sample.Name = {'CrowleyBishopTuff';'CrowleyFilteredBishopTuff';'IckertBishopTuff';'WotzlawFCTMLX';'WotzlawFCTMLXYoung';'WotzlawNutrasCreekDacite';'WotzlawFishCanyonAll';'WotzlawKilgoreTuff';'RiveraAlderCreek';'RiveraAlderCreekA';};
for i=2:length(sample.elements)
    sample.(sample.elements{i}) = NaN(size(sample.Name));
end
sample.ReportedWMAge = [0.7679; 0.7671; 0.7671; NaN; NaN; NaN; NaN; 4.4876; 1.1978; 1.1978]; 
sample.ReportedWMUncertainty = [0.000725; 0.0009/2; 0.0009/2; NaN; NaN; NaN; NaN; 0.0023; 0.0046/2; 0.0046/2]; 
sample.ReportedArAge = [0.7666; 0.7666; 0.7666; 28.201; 28.201; 28.201; 28.201; NaN; 1.1848; 1.1848; ]; 
sample.ReportedArUncertainty = [0.0008/2; 0.0008/2; 0.0008/2; 0.046/2; 0.046/2; 0.046/2; 0.046/2; NaN; 0.0006; 0.0006]; 
sample.Distribution = {'VolcanicZirconLowX';'VolcanicZirconLowX';'VolcanicZirconLowX';'VolcanicZircon';'VolcanicZircon';'VolcanicZircon';'VolcanicZircon';'VolcanicZircon';'VolcanicZircon';'VolcanicZircon';};
% sample.Distribution = {'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';'Uniform';};
...
%     hold on; fill([0, size(data,1)+1, size(data,1)+1,0],[sample.EruptionAge(i)-sample.EruptionAge_Sigma(i)*2, sample.EruptionAge(i)-sample.EruptionAge_Sigma(i)*2, sample.EruptionAge(i)+sample.EruptionAge_Sigma(i)*2, sample.EruptionAge(i)+sample.EruptionAge_Sigma(i)*2],c(3,:),'FaceAlpha',0.1)
