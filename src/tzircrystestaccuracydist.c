...
int main(int argc, char **argv){
	uint32_t distrows, distcolumns, syndistrows, syndistcolumns;
	uint32_t N, i, j, k;
	int world_size, world_rank, rc;

	//Check input arguments
	if (argc != 6) {
		fprintf(stderr,"USAGE: %s <sims-per-task> <nsteps> <dt/sigma> <syn-distribution.tsv> <interp-distribution.tsv>\n", argv[0]);
		exit(1);
	}
	...
	// Import data
	const double* syndist = csvparseflat(argv[4],'\t', &syndistrows, &syndistcolumns);
	const double* dist = csvparseflat(argv[5],'\t', &distrows, &distcolumns);
	...
	for (i=0; i<nNs; i++){
		...
		for (j=0; j<simspertask; j++){
			generateSyntheticZirconDataset(&rng, syndist, syndistrows, tmin_true, tmax_true, uncert, data, N);
