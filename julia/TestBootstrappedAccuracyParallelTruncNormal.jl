...
## --- Generate a synthetic dataset
    ...
    @everywhere function bootstrap_task(jobs, results) # define work function everywhere
        while true
            job_id = take!(jobs)
            out = BootstrapEachN(TruncatedNormalDistribution,Ns,nsteps,burnin,dt_sigma);
            put!(results, out)
        end
    end
    ...
    writedlm(string("BootstrappedEruptionAge_dt_sigma_TruncNormal_", dt_sigma, ".csv"), A, ',')
...
## ---
    ...
    ylims!(0,3)
    savefig(f1,string("AbsoluteError_dt_sigma_TruncNormal", dt_sigma, ".pdf"))

    f2 = plot(Ns,MAD_ExpectedMAD,xlims=[1,1000],xscale=:log10)
    ylims!(0,6)
    savefig(f2,string("Absolute_ExpectedError_dt_sigma_TruncNormal", dt_sigma, ".pdf"))
