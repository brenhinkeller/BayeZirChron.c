...
## ---
    ...
    plot!(range(0,80,length=100),MeltsVolcanicZirconDistribution,label="original")
...
## --- Run MCMC
    ...
    @time metropolis_min(nsteps,dist,ages,uncert,tminDist)
...
## -- Plot some example distributions
    ...
    h = plot(range(0,1,length=length(UniformDistribution)),UniformDistribution./mean(UniformDistribution),label="Uniform")
    plot!(range(0,1,length=length(MeltsVolcanicZirconDistribution)),MeltsVolcanicZirconDistribution./mean(MeltsVolcanicZirconDistribution),label="MELTS Volcanic")
    plot!(range(0,1,length=length(TruncatedNormalDistribution)),TruncatedNormalDistribution./mean(TruncatedNormalDistribution),label="Truncated Normal")
